...
export function Toolbar({
  ...
  let statusText = signedIn ? "Auto-save on" : "Local only";
  if (signedIn && saveStatus === "saving") {
    statusText = "Saving…";
    statusClass += " app-toolbar__pill--busy";
  } else if (signedIn && saveStatus === "saved" && timeLabel) {
    statusText = `Saved ${timeLabel} · Auto`;
    statusClass += " app-toolbar__pill--ok";
  } else if (signedIn && saveStatus === "error") {
    statusText = "Save failed";
    statusClass += " app-toolbar__pill--err";
  } else if (signedIn && hasUnsavedChanges) {
    statusText = `Unsaved · auto-save soon (~${autoSaveAfterSec}s)`;
    statusClass += " app-toolbar__pill--pending";
  } else if (!signedIn && hasUnsavedChanges) {
    statusText = "Local · not saved to Drive";
    statusClass += " app-toolbar__pill--pending";
  } else if (!signedIn) {
    statusText = "Local · sign in to save";
    statusClass += " app-toolbar__pill--pending";
  }

  const statusTitle = signedIn
    ? typeof autoSaveAfterSec === "number"
        ...
        {signedIn ? (
          ...
          <>
            <button
              ...
              onClick={() => onSave()}
              disabled
              aria-label="Save to Google Drive (sign in required)"
            ...
            <button
              ...
              className="btn btn--ghost"
              disabled
              aria-label="Open from Drive (sign in required)"
              title="Sign in to open diagrams from Google Drive"
            >
              Open
            </button>
            <button
              ...
              className="btn btn--ghost"
              onClick={onNewDiagram}
              aria-label="Clear canvas and start a new local sketch"
              title="New blank canvas (local only until you sign in)"
            >
              New
            </button>
            {onShowShortcuts ? (
